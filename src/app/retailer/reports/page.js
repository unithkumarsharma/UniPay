    ...
    <>
      <div className="page-header" style={{ marginBottom: '24px' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '4px 12px', background: 'rgba(37, 99, 235, 0.1)', color: '#2563EB', borderRadius: 'var(--radius-full)', fontSize: '0.8rem', fontWeight: 700, marginBottom: '8px' }}>
          SUPABASE DB • BUSINESS PERFORMANCE AUDIT
        </div>
        <h1 style={{ fontSize: '1.9rem', fontWeight: 900, color: 'var(--text-primary)', letterSpacing: '-0.02em' }}>
          Retailer Business Reports
        </h1>
        <p style={{ color: 'var(--text-secondary)', fontSize: '0.92rem' }}>
          Live breakdown of earnings, transaction volume, and service performance.
        </p>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '20px', marginBottom: '24px' }}>
        <DashboardCard title="Total Volume" value={`₹${totalVolume.toLocaleString('en-IN')}`} subtitle="Processed Sales Volume" change="Live DB" changeType="positive" />
        <DashboardCard title="Commission Earned" value={`₹${totalComm.toLocaleString('en-IN')}`} subtitle="Net Earnings" change="Real Time" changeType="positive" />
        <DashboardCard title="Total Transactions" value={filteredTxns.length.toString()} subtitle="Executed Orders" change="Verified" changeType="neutral" />
      </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
          <span style={{ fontSize: '0.82rem', fontWeight: 700, color: 'var(--text-secondary)' }}>Period Filter:</span>
      ...
      <DataTable title="Report Audit Log" columns={columns} data={filteredTxns} searchable searchField="txnId" />
    </>
  );
}
