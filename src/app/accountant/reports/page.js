...
export default function AccountantReportsPage() {
  ...
  const totalComm = useMemo(() => filteredTxns.reduce((sum, r) => sum + (Number(r.commission) || 0), 0), [filteredTxns]);
  ...
  const columns = [
    ...
    { key: 'type', label: 'Service Category', render: (r) => r.type || 'Transaction' },
    {
      key: 'amount',
      label: 'Volume (₹)',
      render: (r) => (
    ...
    <>
      <div className="page-header" style={{ marginBottom: '24px' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '4px 12px', background: 'rgba(37, 99, 235, 0.1)', color: '#2563EB', borderRadius: 'var(--radius-full)', fontSize: '0.8rem', fontWeight: 700, marginBottom: '8px' }}>
          SUPABASE DB • FINANCIAL AUDIT &amp; P&amp;L REPORT
        </div>
        <h1 style={{ fontSize: '1.9rem', fontWeight: 900, color: 'var(--text-primary)', letterSpacing: '-0.02em' }}>
          Corporate Financial Reports
        </h1>
        <p style={{ color: 'var(--text-secondary)', fontSize: '0.92rem' }}>
          Real-time financial audit, gross volume, commission expenses, and revenue reports.
        </p>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '20px', marginBottom: '24px' }}>
        <DashboardCard title="Gross Transaction Volume" value={`₹${totalVolume.toLocaleString('en-IN')}`} subtitle="Company Sales Volume" change="Live DB" changeType="positive" />
        <DashboardCard title="Total Commissions Paid" value={`₹${totalComm.toLocaleString('en-IN')}`} subtitle="Partner Disbursal" change="Real Time" changeType="positive" />
        <DashboardCard title="Total Audited Transactions" value={filteredTxns.length.toString()} subtitle="Verified Entries" change="Verified" changeType="neutral" />
      </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
          <span style={{ fontSize: '0.82rem', fontWeight: 700, color: 'var(--text-secondary)' }}>Audit Preset:</span>
      ...
      <DataTable title="Financial Audit Log" columns={columns} data={filteredTxns} searchable searchField="txnId" />
    </>
  );
}
